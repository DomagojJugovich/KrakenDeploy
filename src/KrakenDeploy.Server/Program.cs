...
public static class Program
{
    ...
    private static StartupSettings ResolveStartupSettings(WebApplicationBuilder builder)
    {
        ...
        // BG1/T2 — keyed on the topology (the MultiAccount:Enabled switch is gone).
        // Saas has no single tenant DB to read startup settings from: reading an
        // arbitrary tenant's documents would be wrong, so configuration/defaults stand.
        var isSaas = DeploymentTopologyResolver.Resolve(builder.Configuration)
            == DeploymentTopology.Saas;
        var connectionString = isSaas
            ? null
