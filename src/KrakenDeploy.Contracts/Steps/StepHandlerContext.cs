...
/// <para>
/// Part of the stable SDK surface — see <c>docs/sdk-surface.md</c>. Step
/// packages built against an older Contracts version must keep working when
/// the agent ships a newer version; therefore only nullable / defaulted
/// additions land here over time.
/// </para>
/// </summary>
public sealed class StepHandlerContext
{
    ...
    /// entry, keyed by the package's <c>Name</c>. Populated by the executor
    /// after extraction. Handlers should expose these as
    /// <c>OCTOPUS_REFERENCED_PACKAGE_&lt;Name&gt;_PATH</c> env vars and
    /// <c>Octopus.Action.Package[Name].ExtractedPath</c> variables for scripts
    /// that depend on bundled tooling. Empty when the step has no referenced
    /// packages.
    /// </summary>
