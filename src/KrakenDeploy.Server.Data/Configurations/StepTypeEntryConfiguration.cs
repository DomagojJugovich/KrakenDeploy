...
public sealed class StepTypeEntryConfiguration : IEntityTypeConfiguration<StepTypeEntry>
{
    public void Configure(EntityTypeBuilder<StepTypeEntry> builder)
    {
        ...
        builder.Property(x => x.TypeId).IsRequired()
               .HasMaxLength(StepTypeMetadataLimits.TypeId);
        builder.HasIndex(x => x.TypeId).IsUnique();

        builder.Property(x => x.DisplayName).IsRequired()
               .HasMaxLength(StepTypeMetadataLimits.DisplayName);
        builder.Property(x => x.Category).HasMaxLength(StepTypeMetadataLimits.Category);
        builder.Property(x => x.Description).HasMaxLength(StepTypeMetadataLimits.Description);
