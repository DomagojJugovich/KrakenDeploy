...
/// <summary>
/// Archive-layout tests for the produced <c>kraken.iis-*.kdeploy-step</c> —
/// the package shipped with no coverage of its packed shape until the
/// 2026-08-09 version-bump breakage showed the gap. Mirrors the pattern in
/// the other Steps.*.Tests suites so any broken pack target shows up here,
/// not at runtime on a real agent.
/// </summary>
public sealed class KrakenIisStepPackageTests
{
    [Fact]
    public void Built_archive_exists_at_the_expected_path()
    {
        FindBuiltArchive().Should().NotBeNull(
            "the pack target must produce kraken.iis-<version>.kdeploy-step");
    }
    ...
    public void Built_archive_contains_a_well_formed_manifest_and_executor_DLL()
    {
        ...
        manifest.Id.Should().Be("kraken.iis");
        ...
        manifest.StepTypes.Should().HaveCount(2,
            "one handler claims the Kraken-native shape and the Octopus alias");
        manifest.StepTypes.Should().Contain(t => t.Id == "Kraken.IIS");
        manifest.StepTypes.Should().Contain(t => t.Id == "Octopus.IIS");
        manifest.ExecutorTypeName.Should().Be(typeof(KrakenIisStepHandler).FullName!);
        manifest.ExecutorAssembly.Should().Be("KrakenDeploy.Steps.KrakenIis.dll");
    ...
    public void Built_archive_bundles_Steps_Common_and_Octostache_DLLs()
    {
        ...
        zip.GetEntry("executor/KrakenDeploy.Steps.Common.dll").Should().NotBeNull(
            "the handler's process/health-probe plumbing lives in Steps.Common");
        zip.GetEntry("executor/Octostache.dll").Should().NotBeNull(
            "#{Var} placeholders in the IIS config are resolved via Octostache");
    }
    ...
    private static string? FindBuiltArchive()
    {
        ...
        var binRoot = Path.GetFullPath(Path.Combine(
            here, "..", "..", "..", "..", "..",
            "steps", "KrakenDeploy.Steps.KrakenIis", "bin"));
        return Directory.Exists(binRoot)
            ? Directory.EnumerateFiles(binRoot, "kraken.iis-*.kdeploy-step",
