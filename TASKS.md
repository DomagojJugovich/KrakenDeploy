...
- [ ] **M11.E: Ad-hoc agent actions** (natural-language → server-generated PowerShell → operator-approved → agent runs) —
  ...
  - **Iteration loop — operator-approved per-turn (M11.E.12–M11.E.17):**
    The single-shot path above only handles the happy case. When a script errors out on some targets, operators want the LLM to look at the failure output and propose a fix. This is the "B1" pattern from the original threat model — but with **per-target operator approval at every iteration**, it's iterated-B2: each turn is a fresh approval gate, same safety contract as the first turn. No autonomous escalation possible.
  - **M11.E.12** `AdhocSession` + `AdhocIteration` aggregates (Space-scoped) + EF migration. `AdhocSession { Id, Prompt, Mode, Status ∈ {Active, Closed, CapReached, BudgetExceeded, OperatorStopped}, CreatedBy, SpaceId, MaxIterations }`. `AdhocIteration { SessionId, IterNumber, GeneratedScript, ScriptSignature, LlmModel, LlmTokens, ApprovedTargetsJson, ResultsJson, OperatorWhoApproved, ApprovedAtUtc }`. One row per turn; the audit trail is the table itself.
  - **M11.E.13** Iteration LLM call. After each iteration's agent results stream back, server makes a SECOND LLM call with `{ originalPrompt, mode, priorIteration.script, perTargetResults: [{ target, exitCode, stdout, stderr }] }`. Structured output via `IChatClient.CompleteAsync<IterationVerdict>` where `IterationVerdict { Verdict ∈ {AllSucceeded, NoFixAvailable, ProposeFix}, Narrative, ProposedScript?, ProposedScriptDescription?, RecommendedTargets?, RiskAssessment? }`. `AllSucceeded` + `NoFixAvailable` close the session; `ProposeFix` opens the next iteration's approval dialog.
  ...
  - **M11.E.15** Invariants enforced across all iterations: (a) mode immutability — a session started `readonly` cannot have any iteration propose a `mutating` script; the static-analysis gate rejects mode escalation; (b) same allowlist + AST checks on every iteration's script (M11.E.3 applies to v2, v3, … same as v1); (c) cumulative budget enforcement — each iteration's LLM call counts against the Space's monthly cap; (d) per-target isolation — script vN+1 doesn't auto-apply to targets that succeeded on vN; operator approves a specific subset of targets per iteration; (e) signing on every iteration (M11.E.6 applies to v2, v3, … same as v1).
  - **M11.E.16** UI redesign: `/adhoc` page becomes session-based. Each session renders as a card tree: prompt at the root, then one expandable card per iteration. Each iteration card shows the generated script (syntax-highlighted), approved targets (with per-target subset selector — operator can include/exclude targets), per-target results stream, LLM's narrative for that iteration. Three buttons per iteration: `Approve next iteration` (if LLM proposed a fix) / `Stop session` (close with `OperatorStopped`) / `Mark resolved` (close with `Closed`). Audit-trail view per session.
  - **M11.E.17** Tests: round-trip a session that succeeds on iteration 1 (`AllSucceeded`); a session that fixes a target on iteration 2 (`ProposeFix` → operator approves → succeeds); a session that hits the iteration cap (`CapReached`); a session where the LLM tries to escalate from `readonly` to `mutating` (static-analysis gate must reject the iteration's script); a session where the LLM tries to expand the approved-target set without operator action (server enforces operator's subset, ignores LLM recommendation). Property-based test: across 50 randomly-generated multi-iteration sessions, the static-analysis gate trips on every script that contains a forbidden cmdlet, regardless of which iteration produced it.
  - **Out of v1 scope** (documented): autonomous remediation (LLM acting without operator approval at any turn); persistent cross-session agent memory (each session is independent); scripts that propose DIFFERENT remediation per target in a single iteration (one script per iteration, operator picks target subset — if different fixes per target are needed, operator starts separate sessions). These are the remaining high-risk patterns; their absence is intentional.
