...
- [x] **D-12.2: AWS S3 upload sample step package** (done) —
  - `examples/AwsS3UploadStepPackage/` — a non-trivial reference package demonstrating the patterns this guide teaches. Package id `kraken.steps.aws-s3-upload`, step type `Kraken.Steps.AwsS3Upload`.
  ...
  - Test seam: `internal` ctor taking a `Func<string, IS3Uploader>` factory + `[InternalsVisibleTo]` lets the test project drive the handler with a fake uploader. The production ctor news up `NotImplementedAwsS3Uploader` — authors swap in an `AWSSDK.S3`-backed impl before shipping (the README has the 20-line `AwsSdkS3Uploader` snippet). Decision: the sample doesn't pull AWSSDK.S3 into `Directory.Packages.props` — keeps the CI build lean and focuses the sample on handler authoring patterns, not AWS SDK specifics.
  ...
  - `examples/AwsS3UploadStepPackage/README.md` walks readers through: what the sample demonstrates (mapped to authoring-guide concepts), how to swap the uploader for AWSSDK.S3, build + sign + upload commands.
  - 8 tests in `tests/AwsS3UploadStepPackage.Tests/` cover: CanHandle case-insensitivity, missing-config failure, no-match-glob warning (success), happy-path upload + artifact manifest, hard failure aborts batch (default), ContinueOnError tolerates per-file failures, cancellation propagation, missing ExtractDir.
  - `docs/step-packages.md` gets a new "Reference example" section linking to the sample with a mapping table (pattern → location in the handler), and its "Sign your package" section now leads with `kraken pack --key signing.key` instead of the manual 30-line snippet (the snippet is kept further down as a fallback for constrained CI). Local-testing section now mentions `kraken pack` as the production path.
