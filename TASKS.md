...
- [ ] **C-6: Bridge for legacy `StepTemplateParameter`** — community-library step templates (imported via Phase M5 Phase 4) currently use `StepTemplateParameter` with `ControlType` (6-element enum). Add `StepUiSchemaBuilder.FromStepTemplateParameters(IReadOnlyList<StepTemplateParameter>)` that generates a `StepUiSchema` (one property per parameter; widget chosen by `ControlType`; defaults from `DefaultValue`; label from `DisplayName`; helpText from `Description`). Route `TemplatedStepFormDialog` callers through `SchemaDrivenStepFormDialog` so there's one renderer for everything. Keep `TemplatedStepFormDialog` as a thin back-compat wrapper until call sites migrate, then delete.
...
- [ ] **D-2: `Kraken.SDK` NuGet + `dotnet new krakenstep` template** — emit `Kraken.SDK` NuGet from `KrakenDeploy.Contracts` (same assembly source; versioned independently). New project `KrakenDeploy.Templates` shipping a `.template.config/template.json` registers a `dotnet new krakenstep` template: generates a fresh net10.0 class library with `Kraken.SDK` PackageReference, a sample `MyStepHandler : IStepHandler`, a sample `ui-schema.json`, and an MSBuild `<Target Name="PackKrakenStep">` that produces a `.kdeploy-step` zip on `dotnet pack`. Also add a `kraken pack` CLI verb in `KrakenDeploy.Cli` (`kraken pack ./MyStep.csproj --output ./bin/ --key ./signing.key`) that wraps the build, gathers `executor/` + `ui/` + manifest into a zip, and signs.
...
- [ ] **D-4: Agent loader + `AssemblyLoadContext` isolation** — new `StepPackageLoader` service in `KrakenDeploy.Agent`. On first request for `(packageName, version)`:
  - Looks up the cached extraction at `{dataPath}/step-packages-cache/{name}/{version}/`; if missing, triggers gRPC download (D-5).
  - Verifies signature on every load (not just download — guards against on-disk tampering).
  - Creates a new collectible `AssemblyLoadContext` per `(name, version)` pair.
  - **Critical**: configures the ALC to delegate `KrakenDeploy.Contracts` and all `System.*` / `Microsoft.Extensions.*` referenced assemblies to the **default** load context. This prevents the "two `IStepHandler` types from two contexts" identity bug. The ALC loads from its own context only what's in `executor/` (plugin-private deps).
  - Resolves `manifest.executorTypeName` from the loaded executor assembly via `Assembly.GetType`, asserts it implements `IStepHandler`, returns the `Type`.
  - `DeploymentExecutor` constructs an instance via parameterless ctor (v1; a `StepHandlerActivator` indirection can layer DI later), calls `HandleAsync`, disposes if `IDisposable`. **Handler lifecycle is per-step-execution.**

- [ ] **D-5: gRPC channel for step-package delivery** — extend `kraken.proto` (Phase 17) with `rpc DownloadStepPackage(StepPackageRequest) returns (stream PackageChunk)` mirroring the existing `DownloadPackage` shape. `StepPackageRequest { string name = 1; string version = 2; }`. Agent-side `GrpcStepPackageDownloader` in `KrakenDeploy.Agent.Transport`: streams to `{dataPath}/step-packages-cache/{name}/{version}/_downloading.kdeploy-step`, verifies SHA-256 against the manifest hash returned in the final trailer, atomically renames to `.kdeploy-step` on success, extracts to the cache dir, deletes the zip. Server-side `KrakenPackageDeliveryService` extension reads from the existing on-disk zip in `{dataPath}/step-packages/{name}/{version}/`. **Retry / resume**: piggyback on the existing chunk-acknowledgement pattern from Phase 17; no new retry logic.

- [ ] **D-6: Per-step version pinning + snapshot pinning** —
  - New column `DeploymentStep.StepPackageVersion : string` (non-nullable on new rows; nullable for the migration window). EF migration `AddDeploymentStepPackageVersion`.
  - New column `StepSnapshot.StepPackageVersion : string` (same migration profile). EF migration `AddStepSnapshotPackageVersion`.
  - `ProcessService.AddStepAsync` accepts and persists a `stepPackageVersion` parameter; when unspecified it resolves to the latest installed version of the package whose `manifest.stepTypes` contains the step's `StepType` (or throws when no installed package claims that step type — fail loud).
  - `ProcessService.UpdateStepAsync` accepts an optional override.
  - `ReleaseService.CreateAsync` populates `StepSnapshot.StepPackageVersion` from `DeploymentStep.StepPackageVersion` at snapshot time; if the deployment step's version is somehow null at snapshot creation, re-resolves to the latest installed at that moment and locks the result.
  - `DeploymentStepPlan` (wire contract to the agent) gains a `StepPackageVersion` string field (nullable append for back-compat during the migration window).
  - `DeploymentExecutor` reads `StepPackageVersion` from the plan and asks `StepPackageLoader` for that exact version.
...
- [ ] **D-9: GitHub-feed sync** — new repo (`KrakenDeploy/StepPackages` or similar) hosts canonical `.kdeploy-step` releases under GitHub Releases. New `StepPackageCatalogService` in `KrakenDeploy.Server.Data.Services` polls the GitHub Releases API hourly via a Hangfire recurring job `kraken.step-package-catalog-poll` (mirrors `StepTemplateCatalogService` from Phase M5 Phase 4 — same `HttpClient` + named `kraken.github` client + optional `GitHub:Token`). For each release: reads `manifest.json` from the release notes (embedded as a fenced JSON block) or from the asset's manifest file. Persists `StepPackageCatalogEntry` rows: `Name`, `Version`, `DownloadUrl`, `Sha256`, `ManifestJson`, `Changelog`, `PublishedUtc`. EF migration `AddStepPackageCatalog`. Endpoints `GET /api/step-package-catalog`, `POST /api/step-package-catalog/refresh`, `POST /api/step-package-catalog/{name}/{version}/install` (downloads the asset, verifies signature against project key, calls into D-3 storage path). UI: extend `/step-packages` with a "Catalog" tab showing available-but-not-installed entries grouped by name, with one-click install. Air-gapped installs continue to use D-3's manual upload.

- [ ] **D-10: Bulk-upgrade admin tool** — new `/step-packages/{name}/usage` page listing every `(project, process, step, currentVersion)` row across the whole instance, grouped by `currentVersion`. Multi-select grid lets the admin pick rows and apply "Bump to version X" in a single transaction. Pre-apply preview: schema delta + per-step incompatibility flags (e.g. "step uses field `OldName` which no longer exists in the new schema — value will be dropped"). Audit log entry per touched step (using existing M10 RBAC audit infrastructure). Released snapshots are *not* touched.

- [ ] **D-11: Uninstall + conflict detection** — `DELETE /api/step-packages/{name}/{version}`. Refuses with HTTP 409 + a `ConflictReport` body listing every `(project, process, step)` and every `StepSnapshot` (release) referencing that version. UI presents the conflict report; the admin can: (a) cancel; (b) bulk-upgrade live rows via D-10 (released snapshots stay pinned and cannot be re-pointed — the version stays uninstall-blocked until those releases are deleted or retention-pruned). Successful uninstall deletes the row, removes `{dataPath}/step-packages/{name}/{version}/` from disk, and emits an audit event. Agent-side cached copies are *not* actively purged — they sit until cache TTL or manual cleanup (separate `kraken cache prune` CLI verb).

- [ ] **D-12: Authoring guide + dev experience** — new doc `docs/step-packages.md` covering: project template, manifest fields, `IStepHandler` contract, schema authoring (attributes vs JSON), signing setup (development key generation + signing flow), local-dev signing key acceptance via the dev-mode allowlist from D-3, building + packaging via `dotnet pack` or `kraken pack`, testing locally against a Kraken server (upload via `curl` or `kraken pkg upload`), publishing to the GitHub feed (release flow: tag commit → GitHub Actions builds + signs the zip with the project key from repo secret → attaches `.kdeploy-step` to the GitHub Release → catalog poll picks it up within the hour). Sample repo (separate from main Kraken repo): two examples — a minimal "Hello World" step and a non-trivial "AWS S3 upload" step demonstrating async work, log streaming, and the artifacts directory.
