...
- **Rolling deployments** — M-RollingDeployments milestone. Not a new step type — just `Octopus.Action.MaxParallelism` on a `Kraken.StepGroup` driving a target-fan-out pass in the orchestrator. Requires multi-target Deployment row (the structural blocker called out in M14's "deferred" section). M15's machinery is the foundation; M-RollingDeployments adds the property-consumption pass.
  ...
  **Phase 3 — Polish, DONE 2026-05-26**:
  ...
  - **Test surface**: no new unit tests for the drop-out logic — the change is end-to-end orchestrator behaviour and there's no E2E orchestrator harness (known gap since M14.2). The 879 baseline holds; manual verification + careful diff review is the safety net. The audit-event constants + UI render path compile-test through the existing suites.
  - Suite: 386 + 225 + 135 + 133 = **879 green** (same as Phase 2 baseline; no new tests).
