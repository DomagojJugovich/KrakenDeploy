...
- [x] **D-12: Authoring guide + dev experience** (done) —
  ...
  - **Deferred** (documented as follow-ups, none blocking D-track completion):
    - `kraken pack` CLI verb in `KrakenDeploy.Cli` (the MSBuild pack-on-build already covers the same ground; CLI verb makes sense once it adds `--key ./signing.key` ergonomics).
    - `StepPackage.ChangelogMarkdown` column extracted from `CHANGELOG.md` at upload.
    - Sample step-package repo with a non-trivial "AWS S3 upload" example demonstrating async work, log streaming, and the artifacts directory.
    - GitHub Actions workflow YAML (lives in the separate `KrakenDeploy/StepPackages` repo).
    - MSBuild signing integration (currently manual via `StepPackageSigner` API).
