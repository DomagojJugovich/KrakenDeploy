...
| ~~**K** — Direct + Polling transports~~ | removed | Prototyped in `c66b072`, then **removed 2026-06-16**: KrakenDeploy is SignalR-only for live agents (`SignalRServerLink`). Air-gapped targets use the agentless **OfflineDrop** path. `DirectServerLink`/`PollingServerLink` and the `Direct`/`Polling` `TransportMode` members are gone. |
...
**Transport modes — DECIDED 2026-06-16: SignalR-only + OfflineDrop.**

- Live agents use **SignalR only** (`SignalRServerLink`, "Reverse" mode): the agent opens one outbound full-duplex connection and the server pushes work back down it. This covers both firewall/NAT and flat-LAN cases, so a server-initiated "Direct" path adds nothing but an inbound listener + its own TLS/auth burden.
- Air-gapped targets that can't dial out use the agentless **OfflineDrop** path (server emits a bundle, run manually, upload the result).
- ~~Add `DirectServerLink` / `PollingServerLink`~~ — **won't do.** Both were prototyped (milestone K) and removed; the half-built `DirectServerLink` also shipped an unauthenticated inbound listener (RCE surface). The `TransportMode` enum and the Add-Target wizard now offer only **Reverse** vs **OfflineDrop**.
...
- [ ] **Fleet migration trigger** *(planning — flagged 2026-07-10)*: `FleetMigrationOrchestrator.MigrateAllAsync` exists and is DI-registered (`ControlPlaneServiceCollectionExtensions.cs:51`) but has **no caller** anywhere — no startup hook, no CLI verb, no admin endpoint. Consequence: in multi-account mode, **already-provisioned tenant DBs do not receive new EF migrations**. Single-instance `krakendeploy_dev` auto-migrates on dev startup (`Program.cs:657`), and `ControlPlaneDevSeed` migrates only *newly* provisioned accounts (it skips accounts that already exist), so live tenants silently drift. Surfaced while shipping the `DropSubscriptionPollerStateAuditColumns` migration (the subscription-poller audit-loop fix): it reached `krakendeploy_dev` on startup but would **not** reach the `acme`/`globex` tenant DBs. **To plan:** trigger mechanism (CLI `database migrate-fleet` vs. control-plane admin action vs. gated startup sweep), advisory-lock + ordering, and how `FleetMigrationReport` per-account failures surface/alert. Ties into the blue/green item above (backwards-compatible migrations across one version).
...
- [x] **`Octopus.Manual`** — `ManualInterventionStepHandler` aligned with the Octopus public contract. Reads `Octopus.Action.Manual.Instructions` (Octostache-evaluated), `Octopus.Action.Manual.ResponsibleTeamIds` (logged for audit), `Octopus.Action.Manual.BlockConcurrentDeployments` (logged + ignored — F1 already serializes deployments per project+environment+tenant unconditionally, which is stronger). **Superseded by WP3**: the real pause/approve/reject gate is server-side; this handler is the no-server fallback and now warns `APPROVAL NOT ENFORCED`. Legacy un-prefixed `Instructions` key honoured for back-compat. New `OctopusManualConfigKeys` constants alongside the handler.
...
- [x] **M11.B: MCP server** — **DONE 2026-05-27** (6 commits: `42dd02e` skeleton + gate, `6a8306b` curator kernel, `e3f4d02` resources, `4e1f45b` context builders, `4c4dc4c` tools, `4051c8b` stdio proxy, `<this>` integration test + docs). Shipped: `src/KrakenDeploy.Mcp/` (in-process Streamable-HTTP server at `/mcp`, API-key auth + per-Space `McpEnabled` gate middleware), the shared context-builder kernel in `Server.Data/Services/Ai/` (Process/Deployment/TargetHealth/Release/Diff builders + `IStepConfigCurator` registry — also feeds M11.C diagnosis), **7 resources + 8 tools** (all audited via `Mcp.ResourceRead` / `Mcp.ToolInvoked`), the `kraken-mcp` stdio↔HTTP proxy exe (`src/KrakenDeploy.Mcp.Cli/`), an in-process client↔server integration test, and `docs/mcp.md`. **Deferred** (documented in `docs/mcp.md`): granular per-key `DeploymentExecute` gate on `retry_deployment` + multi-Space scoping wait on M13.C.4 per-user API keys; `artifacts/{name}` + `step-packages/{name}/{version}/manifest` resources not yet shipped (no tool needed them). Suite +37 tests → **922 green**.
...
- [x] **M11.C: Autonomous failure diagnosis** — **DONE 2026-05-27**. Shipped: `DeploymentDiagnosis` aggregate + `AddDeploymentDiagnosis` migration; `DiagnosisContextAssembler` + `DeploymentDiagnosisService` (reuse the M11.B context-builder kernel — log tail + diff + failed-step curated config + target health — and call `IKrakenAi.CompleteAsync<DiagnosisResult>` structured output, sanitising decrypted Sensitive snapshot vars); trigger via a `DeploymentDiagnosisChannel` (typed channel, mirrors `RunbookRunChannel`) written by `DeploymentWorker.FailAsync` ONLY when `StartedUtc` is set (skips pre-flight refusals — no AI budget wasted on "it never ran"), drained by `DeploymentDiagnosisWorker` (BackgroundService, best-effort, off the deployment hot path — no Hangfire dep in Transport); `Diagnosis.Completed` audit (same event the M13.B AiInspect transport emits, so "diagnose → notify" subscription chains keep working); "AI Analysis" card on `DeploymentDetail.razor` (probable cause + confidence badge + suggested fix + relevant log lines + Low-confidence "verify yourself" footer). **Deviations from the original plan:** (a) **channel + BackgroundService instead of a Hangfire job** (M11.C.3) — keeps the trigger out of the Transport layer + matches the existing dispatch-channel pattern; (b) **M11.C.5 webhook push is superseded** by M13.B subscriptions — subscribe to `Diagnosis.Completed` + route via the Webhook transport (no separate `Notifications:Failure:WebhookUrl`); (c) coexists with M13.B's `AiInspectTransport` — that's a generic event→free-text subscription, this is automatic rich structured diagnosis on deployment failure with a persisted aggregate + UI card. AI-unavailable / over-budget / transient errors never affect deployment status (caught + logged). 16 tests (7 service + 1 orchestrator trigger + assembler/builder coverage). Suite → **930 green**.
...
- [x] **M11.E: Ad-hoc agent actions** (natural-language → server-generated PowerShell → operator-approved → agent runs) — **DONE 2026-05-29**. Shipped as 7 commits across all sub-tasks 1–17. Architecture: B2 script-handoff (locked) — `AdhocSession` + `AdhocIteration` aggregates with immutable `FrozenTargetSetJson`, `Permission.AdhocActionsExecute=1900`, EF migration; AST static-analysis gate on `System.Management.Automation.Language.Parser` (readonly = Get/Test/Measure + curated safe-utility allowlist, mutating = forbidden-blocklist incl. `Invoke-Expression`/`Add-Type`/`Invoke-Command -ComputerName`/`Remove-Item -Recurse -Force`/service install-uninstall/registry writes; both modes reject dynamic invocation + parse errors); generation pipeline + `Adhoc:SigningKey` signing (separate key from step packages; canonical input binds signature to `(sessionId, iterNumber, scriptBytes)` so signatures can't replay across sessions/turns); dispatch via new `IAgentHubClient.RunAdhocScriptAsync` (SignalR primary; Direct/Polling carry TODO stubs matching the M14.4 pattern) + per-target `AdhocPerTargetResult` collation; agent-side `AdhocScriptExecutor` is fail-closed (missing key / malformed key / signature mismatch / runtime exception → REFUSE + report `AgentError`); iteration loop with verdict LLM + cap enforcement (default 5, configurable via `Ai:Adhoc:MaxIterationsPerSession`); `/adhoc` Radzen UI (sessions list + per-session card tree + approve/edit-and-approve/reject dialog, gated by `AdhocActionsExecute` + per-Space `AdhocEnabled`); `run_adhoc_action` + `get_adhoc_session` MCP tools that CANNOT auto-approve (`ApprovalPending=true` + deep-link URL — the operator approves in `/adhoc/{id}` regardless of source). **Documented limitations:** static .NET method calls (e.g. `[System.IO.File]::Delete`) + variable-resolved file paths are NOT statically guaranteed — defence-in-depth (operator approval + signing + frozen targets + mode immutability + cap) covers residual risk. **v1 deviations:** per-Space iteration-cap override is currently a global config key (the per-row `AdhocSession.MaxIterations` is in place for the per-Space surface to land later); `RiskLevel ∈ {Dev,Staging,Production}` on targets + the louder production-mutating banner (M11.E.11) deferred (UI is structured to light up when the field ships); two-person rule (M11.E.5/M11.E.11) deferred behind a feature flag. SystemManager + SpaceManager built-in roles carry `AdhocActionsExecute`; explicit grant required for other teams. See `docs/adhoc-actions.md`. Suite → **1028 green** (Data 517 + Server 146 + Core 226 + Agent 139; +94 over the 934 baseline for M11.E: gate property-based 50-session × 1-5 iterations, dispatcher frozen-set invariant, signer roundtrip + replay-prevention, agent verify fail-closed paths, full state-machine round-trip + cap + escalation reject, MCP permission gate + ApprovalPending contract).
...
- [x] **M11.D: Process builder assistant (UI)** — **DONE 2026-05-27**. Backend `ProcessAssistantService` (Server.Data/Services/Ai/Assistant) with three ops under `KrakenAiFeature.Assistant`: `SuggestStepsAsync` (opens the package zip via `IPackageStore`, collects layout signals, structured `StepSuggestionResult`), `ExplainFieldAsync` (one-shot field explanation), `StreamScriptSuggestionAsync` (streams via `IKrakenAi.StreamChatAsync`). UI: **D.1** "Suggest from package" button on `Process.razor` → `SuggestProcessDialog` (pick package/version → suggestions with per-step tick → create via `AddStepAsync`); **D.3** "Explain a field" block in `StepFormDialog` (field-picker + explain, per-dialog cache); **D.2** streaming "Script assistant" block in `StepFormDialog` for script step types (explicit Generate button + "Use as script body"). All gated on the per-Space `AssistantEnabled` flag. **Deviations (documented in code):** D.3 uses a single field-picker block rather than a per-field icon (the schema body is a manual RenderTreeBuilder — per-field icons would be fiddly/error-prone); D.2 uses an explicit Generate button rather than debounced-on-keystroke (predictable, no wasted LLM calls per keystroke). 4 backend tests (fake `IKrakenAi` + fake package store); UI is compile-verified (no Blazor runtime harness, consistent with the rest of this session's UI). Suite → **934 green**.
...
- [x] **M13.B: Notifications & event routing** — **DONE** (5 phases shipped across commits `12e69c1` SMTP, `bd8c36e` foundation, `5fc2f5d` poller+webhook, `93020ea` Runbook+AI+Email-immediate, `cc243ba` UI, Phase 5 digest+seed).
  - **M13.B.1** ✓ SMTP config + Save-and-test (commit `12e69c1`).
  - **M13.B.2/3** ✓ Subscriptions — Octopus-style outbox-poller event routing (no sync-publish coupling to `IAuditLog`), Space-scoped + nullable for system-wide. **Four transports**: Webhook (HMAC-signed Octopus-shaped payload), Runbook (KrakenDeploy-specific differentiator — calls `RunbookService.TriggerAsync`), **AI Inspect (closes M11.C** — diagnosis written as `Diagnosis.Completed` audit event that's itself subscribable; chains "diagnose, then webhook" with two stacked subscriptions), Email (immediate per-event OR digest mode with `DigestEveryMinutes` window batching, 100-event cap per digest per Octopus parity). UI at `/configuration/subscriptions` + per-subscription delivery-history sub-grid. `EventSubscriptionService.EnsureDiagnoseDeploymentFailedAsync` provides the M11.C built-in seeder. Polling cadence 1 minute (Hangfire). Idempotency via UNIQUE `(subscription_id, event_id)` on `subscription_deliveries` table.
...
- [ ] **M13.F: Features & Behaviour** (lower priority; mostly nice-to-haves) —
  ...
  - **M13.F.3 / F.5 (Large plan, ~half day)** — three coordinated pieces. Scope chosen 2026-05-23 after a codebase survey rejecting several "tempting but wrong" knobs.
    - **F.3 Performance settings** at `/configuration/performance`: DB-backed `PerformanceSettings` singleton (same pattern as `SmtpSettings` / `BackupSettings` / `MaintenanceSettings`). Three knobs:
      - `HangfireWorkerCount` (default 4, currently hardcoded in `Program.cs`) — direct CPU/memory dial; operators on bigger boxes want 8-12. Read at startup; restart required for the change to take effect (Hangfire's worker count is a builder-time setting, not runtime — surface this in the help text).
      - `SlowDeploymentThresholdMinutes` (default 30) — when a deployment exceeds N minutes, the `DeploymentWorker` emits a new `Deployment.Slow` audit event. Plugs into M13.B.2/3 subscriptions trivially (operator subscribes to `Deployment.Slow` → webhook / email).
      - `SlowStepThresholdMinutes` (default 10) — same shape, emits `DeploymentStep.Slow` from the agent's step-completion path back to the server.
    - **F.5 New Features-panel toggles** (4 added to `BuiltInFeatureCatalog`):
      - `security.allow-oidc-sign-in` (default ON) — master kill-switch for every configured `IdentityProvider`. `OidcRegistrar` consults the flag; SSO turns off without deleting the IdP rows. Useful during incident response.
      - `security.show-error-stack-traces` (default OFF) — show full exception stacks on the global `blazor-error-ui` boundary + API error responses. **Off in prod (LAUS context)**; on in dev/staging.
      - `audit.purge-enabled` (default ON) — surfaces the existing `Retention:AuditLogDays = 0 disables` rule from M13.F.4 as a visible operator choice on the Features page. When toggled off, `AuditRetentionJob` short-circuits regardless of the day-count setting.
      - `ui.show-advanced-step-fields` (default OFF) — power-user step-editor fields (raw JSON config, run-on-server toggle, condition expressions). Off reduces cognitive load for typical users; sysadmins flip on.
    - **Retention card on the Performance page** — promote `Retention:AuditLogDays` + `Retention:AiCallLogDays` from `appsettings.json`-only to a UI card with current-value + slider. Forms one cohesive "Retention" section with the `audit.purge-enabled` kill-switch from F.5.
    - **Deliberately rejected (with rationale)** — to keep the page from drifting toward "every config key is a knob":
      - gRPC `ChunkSize = 64 KB` (4 sites in Transport) — network-card-shaped; operators who'd tune it edit YAML; UI invites cargo-culting.
      - Agent polling intervals (heartbeat 30s, poll 5s, offline-threshold 3 min, SignalR grace 30s) — correctness-sensitive (heartbeat-too-short → flapping; offline-threshold-too-short → false alarms). Hardcoded with documented values is the safer posture.
      - `MaxEventsPerCycle = 500` / `MaxEventsPerDigest = 100` — operational caps, not capacity dials. Operators don't have intuition for tuning them.
      - Octopus's Channels / Multi-Tenant feature toggles — KrakenDeploy ships them as always-on domain features (Channels + Tenants are real models, not optional). Toggling off would hide working UI.
      - `telemetry.send-anonymous-stats` — no telemetry-to-external exists today (only `OpenTelemetry.Exporter.Console`); adding a toggle for a non-existent feature is noise.
...
3. **M13.C.6** Filter-state callouts — cosmetic sweep across 4 list pages. 1h. **DONE** — Users (landed with M13.C.3), Teams (new "Hide built-in" toggle with callout — built-in team rows clutter the list for operators managing their own teams), Audit ("Filters active: event=X, user=Y, subject=Z. Showing N matching entries." callout when any free-text filter is set; date-range not counted because the page initialises it to last 30 days and noting that on every visit would be noise). API Keys page deferred — it doesn't exist yet (that's M13.C.4). Pattern is consistent: blue Info alert with filter_alt icon, plain-language sentence, single "Clear / Reset / Show all" button.
4. **M13.F.4** Configurable retention window + `AiCallLogRetentionJob`. 1-2h. **DONE** (commit `de9868a`).
5. **M13.E.3** Quota enforcement — call existing `LicenseService.GetLicenseWarning` from write paths. 1h. **DONE** — `ILicenseGate` introduced in `Server.Core/Domain/Licensing`; `LicenseService` implements it; `TargetRegistrationService.CreateAsync`, `UserService.InviteAsync`, and OIDC JIT-provisioning gate against it. `LicenseWarningBanner` now reads real counts via `LicenseUsageCounter` (60 s cache). 4 integration tests pin the contract. Spun up `tests/KrakenDeploy.Server.Tests` for unit tests of the gate's arithmetic (19 tests, including cap-zero-means-unlimited and expiry-outranks-cap edge cases).
6. **M13.E.2** License Usage dashboard — wire real counts + add per-Space rollup UI. 2-3h. **DONE** — `/configuration/license-usage` shipped. Gauges for Targets + Users with green/info/amber/red thresholds (matching the gate's 90% / 100% boundaries); per-Space target rollup table with share-of-total bars; "Other resources" card (Projects, Tenants, Environments, Spaces — uncapped, here for capacity-planning context). Reuses `LicenseUsageCounter` (extended with `GetPerSpaceTargetCountsAsync` + `GetOtherResourceCountsAsync`). Two new shared components: `UsageGaugeCard`, `ResourceCounter`. Nav link in Configuration group.
7. **M13.E.1** License page polish — parsed display + audit on save. 1h. **DONE** — added `LicenseService.SaveAndActivateAsync` (validates + persists to `data/license.key` + clears cache; refuses when `KRAKEN_LICENSE_KEY` / `License:Key` config override is set — silently writing a file the runtime would ignore is worse than refusing). Razor page now (a) persists on Activate so the key survives restart, (b) writes `License.Uploaded` audit event with a sanitised summary (`LicenseService.FormatAuditSummary` — customer + type + expiry + caps, **never the raw JWT**), (c) writes `License.UploadRejected` audit event on invalid paste for forensic trail, (d) invalidates `LicenseUsageCounter` so the banner updates immediately, (e) "View usage" shortcut to `/configuration/license-usage`. 10 new unit tests including the "invalid paste doesn't overwrite existing file" invariant and the "summary never leaks JWT-shaped content" property.
8. **M13.A.1** Audit export buttons. 2h. **DONE** — `AuditExportService` in `Server.Data.Services` streams filtered audit rows to caller-supplied output stream in CSV (RFC 4180, UTF-8 BOM so Excel auto-detects Croatian diacritics) or JSON (array of objects with inlined Before/After snapshot objects — consumers parse the row in one pass, not two). Two minimal-API endpoints `/api/audit/export.csv` + `/api/audit/export.json` gated on `Permission.EventView`. `Audit.razor` got CSV / JSON buttons in the filter bar that build the URL from the current filter state — exports exactly what's on screen. 27 tests (16 CSV-escape unit + 8 integration + 3 misc): RFC 4180 corners (quote, comma, CRLF, tab-doesn't-trigger), DB round-trip through EF AsAsyncEnumerable, IgnoreQueryFilters preserves cross-Space view, From/To boundary matches page's exclusive-To semantics.
9. **M13.C.3** Service-account discriminator on `ApplicationUser`. 2h. **DONE** — `UserKind` enum in `Server.Core/Domain/Security` (Human=0, ServiceAccount=1); `ApplicationUser.Kind` column via migration `AddUserKindDiscriminator` (defaults existing rows to Human). `UserService.CreateServiceAccountAsync` mints `svc-{slug}` accounts with synthetic `@kraken.local` emails and NO local password (Identity's password sign-in refuses naturally because the hash is null). `OidcRegistrar` refuses SSO sign-in if a found user is `UserKind.ServiceAccount` — closes the "IdP user whose email collides with a service account inherits its team membership" escalation. Users.razor: "Kind" column with chips, segmented filter All/Humans/Service accounts, "New Service Account" button, filter-state callout when toggle is non-default. Service accounts count against `MaxUsers` (pinned by test). 8 new tests covering the discriminator, slugification, license-cap combined count, and the empty-slugify rejection edge case.
10. **M13.A.3** Maintenance mode. 3h. **DONE** — `MaintenanceSettings` singleton with `Enabled` + `Reason` + audit metadata. New permission `BypassMaintenance` (1800-block), explicitly granted to SystemManager so the delegated-admin tier can still run the work itself. `MaintenanceMiddleware` blocks POST/PUT/PATCH/DELETE with 503 + JSON body unless caller has bypass; GET/HEAD/OPTIONS always pass; exempts `/login`, `/logout`, `/_blazor`, `/_framework`, `/_content`, `/api/agents`, `/healthz`, `/api/diagnostics`, `/hangfire`, `/configuration/maintenance`, `/api/maintenance` (without those exemptions the gate locks operators out + flips monitoring red + drops every agent offline). `MaintenancePause` helper wires into BackupJob + SubscriptionPollerJob + EmailDigestFlushJob so recurring jobs short-circuit cleanly during the window. Fail-open posture on Hangfire (DB blip won't pause backups indefinitely); fail-closed posture on HTTP (security boundary). UI at `/configuration/maintenance` with reason textarea + status banner + on/off toggle. Two audit events `Maintenance.Enabled` / `Maintenance.Disabled`. 16 new tests (8 service + helper, 8 middleware path-routing) including case-insensitive prefix match + per-job pause integration.
11. **M13.F.3 + F.5 (Large plan)** Performance knobs + 4 new Features toggles + Retention UI card. ~half day. See the M13.F.3 entry above for the full scope + the deliberately-rejected list of "tempting but wrong" knobs (gRPC chunk size, polling intervals, operational caps).
12. **M13.A.2** Diagnostics page. Half day. **DONE** — `/configuration/diagnostics` page with three operator surfaces: (a) server-info card (.NET runtime, OS+arch, environment name, uptime, working set, threads, started-at, connected agents / total targets, DB version + reachability, data-path size, row counts across every major DbSet); (b) "Run integrity check" button — pending-migrations check, spaces-table-non-empty check, cross-Space orphan scan, license-presence info; (c) "Download diagnostics report" → ZIP with `server-info.json` + `integrity-check.txt` + `config.sanitised.json` + `recent-log.txt` (last 1000 lines of `logs/server-*.log`). The zip is safe to attach to a public support ticket — `DiagnosticsService.SanitiseConfig` redacts any config key matching Password/Secret/Key/Token/ApiKey/MasterKey + value-level password strip on connection strings. Reuses `IAgentConnectionRegistry` (per the audit note) and links to `/hangfire` instead of rebuilding job-level introspection. New shared `KeyValueRow` component for the consistent label/value layout. 26 new unit tests cover the redaction contract end-to-end — including a JWT-shape sniff to catch any future sensitive key that slips past the classifier.
...
21. **M13.G** Backup & Restore — UI page + Hangfire schedule + IBackupTarget. ~1 day (CLI core is already done — `BackupCommands.cs` + `RestoreCommands.cs` exist). **DONE (UI + schedule + history)** — `BackupEngine` extracted from the CLI's `BackupCommands` (in-process version of the same pg_dump + data-copy + manifest pipeline; bundle layout unchanged so existing restore tooling stays compatible). `BackupService` wraps the engine + persists each run to a new `backup_runs` table (in-flight row created before engine call → updated on completion). `BackupSettings` singleton-row aggregate (TargetDirectory, ScheduleCron, ScheduleEnabled, RetainLastN). `BackupJob` Hangfire wrapper + `BackupScheduler` helper that AddOrUpdates/RemoveIfExists the recurring job based on settings — registered at startup AND after every settings save. `/configuration/backup` Razor page: encryption-master-key warning callout up top (M13.G.4), settings form with cron + retain-N + on/off switch, "Run backup now" + "Save settings" buttons, "Last successful backup" banner, recent-runs grid with success/in-progress/failed badges + bundle path or error message. Three new audit event types: `Backup.SettingsUpdated`, `Backup.Completed`, `Backup.Failed`. **IBackupTarget abstraction (M13.G.2) + restore-from-UI deliberately deferred** — restore-from-UI in a running orchestrator is too dangerous (can't restore yourself while running); CLI workflow stays the source of truth. 11 new tests pin settings-CRUD trimming + validation, run-persistence-even-on-failure, in-flight-row contract, history ordering, and last-successful-ignores-failed.
...
**M15.2 — Orchestrator pre-flatten + ForEach semantics — DONE 2026-05-25**

Landed:
- New `DeploymentPlanFlattener` (Server.Transport, alongside `DeploymentWorker`). Pure-function `Flatten(snapshotSteps, arrayVars, scalarVars) → (Plans, SnapshotByPlanIndex, Warnings)`. Single recursive walker (`EmitStep`) handles all modes via Config inspection.
- Modes implemented: plain container (children emitted in SortOrder, group's StartTrigger applies to first child); ForEach (iteration variable + index variable injection, synthetic naming, per-iteration Octostache substitution); empty collection → `WarningKind.ForEachEmpty`; undefined collection → `WarningKind.ForEachUnresolved`; nested ForEach with innermost-shadowing; lazy collection resolution per outer iteration (inner `Collection = "#{env}-instances"` resolves to `staging-instances` then `prod-instances`); parallel ForEach (`Octopus.Action.ForEach.Parallel = "true"` emits iterations 1..N first-children with `StartWithPrevious`).
- `DeploymentStepPlan.AccumulatorKey` added (back-compat default `null`; positional at end of record). Synthetic naming: `AccumulatorKey = OriginalName[index]` (always, stable); display `Name = OriginalName [var=value]` clean (≤ 40 chars, no `\n`/`\t`/`]`) or `OriginalName [var=#index]` fallback.
- **Octostache substitution moved from `DeploymentWorker.SubstituteConfig` into the flattener** — substitution runs per emitted plan with the per-iteration variable bag layered over the deployment-level scalar bag (iteration vars shadow scalars). `SubstituteConfig` method removed from `DeploymentWorker`.
- `StepSnapshot.Id` added (frozen at cut time as `DeploymentStep.Id`) so `ParentStepId` actually links to something. Pre-M15.1 snapshots deserialise with `Id = Guid.Empty` → treated as flat top-level (matching pre-M15 runtime).
- `ReleaseService.CreateAsync` + `RunbookService` snapshot creation now copy `Id` (+ `ParentStepId` for releases; runbooks stay flat since `RunbookStep` doesn't carry it yet — ~~documented as a follow-up if runbook composition is needed~~ **resolved 2026-05-26**, see below). This was a gap from M15.1 — the M15.1 commit added the snapshot fields but didn't wire the snapshot creators.
- `DeploymentWorker.DispatchAsync` rewritten around the flattener: pre-flatten → run `PackageReferenceResolver` per plan (post-flatten, since it's async + DB-backed) → process `Warnings` (ForEachEmpty: log + audit + continue; ForEachUnresolved: log + audit; Required → `FailAsync`, non-Required → continue with `hasFailed`). Every `snapshotSteps[s.Index]` lookup switched to `snapshotByPlanIndex[s.Index]` since plan Index is now the emission-order (not the original SortOrder rank).
- Agent: `DeploymentExecutor.RunStepInWaveAsync` reports outputs by `step.AccumulatorKey ?? step.Name`. `outputsByStep` accumulator indexes by accumulator key so `Octopus.Action[Deploy[0]].Output.X` references resolve. Pre-M15 plans (AccumulatorKey null) fall through to `step.Name` — behaviour unchanged.
- 2 new audit events: `Deployment.ForEachEmpty`, `Deployment.ForEachUnresolved`.

Decisions made during implementation:
- **No synthetic failing step plan** for ForEachUnresolved. The M15.2 plan body proposed emitting a synthetic step "so the Required gate decides" — but that would need either a new server-only step type + dedicated runner, or a hack on an existing handler. Cleaner approach: the flattener returns a warning; the orchestrator's warning loop reads the group's snapshot Required flag directly. Same outcome (Required → abort; non-required → continue) with less machinery.
- **Cross-iteration access form documented as synthetic-key**: `#{Octopus.Action[Deploy[0]].Output.X}` is the supported form (matches what the agent + server write). The M15 plan body had two forms (with the index in a separate `[N]` segment); the synthetic-key form is what falls out naturally from the implementation and is what gets documented. The double-bracket form would need additional Octostache extension and adds little value.
- ~~**Runbook composition not enabled in M15**. `RunbookStep` doesn't have `ParentStepId`; M15 leaves runbooks flat.~~ **Resolved 2026-05-26 in the runbook-composition follow-up commit** — `RunbookStep.ParentStepId` + self-FK + migration + snapshot copy + `RunbookService.AddStepAsync` / `UpdateStepAsync` accept `parentStepId` / `UpdateParent`. `RunbookRunWorker` now pre-flattens via `DeploymentPlanFlattener` so ForEach + Step Groups work at run time. The validator generalized through a new `IComposableStep` interface (DeploymentStep + RunbookStep both implement it; `ProcessValidator.Validate` takes `IEnumerable<IComposableStep>` and works for both via `IEnumerable<>` covariance). The `UpdateParent` record moved from `ProcessService` into `Server.Core.Domain.Processes` so both services share it. ~~RunbookDetail.razor editor UI (tree view + Step Group picker on the inline form) is OUT of scope — the API path supports composition; UI parity with Process.razor is a separate follow-up.~~ **UI parity resolved 2026-05-26 in the StepFormDialog-runbook-migration follow-up commit** — the runbook editor's inline form is gone; the unified `StepFormDialog` flow (same as the process editor) handles Step Group creation, ForEach panel, parent dropdown, step-package version pin, schema-driven body, and ChooseStepTemplateDialog picker. Achieved via new `IStepEditingHost` interface that `ProcessService` + `RunbookService` both implement, with a `SupportsExecutionKnobs` capability flag so the M14 knobs card hides on the runbook editor (RunbookStep has no M14 fields).

Tests (+14 in Server.Tests): `DeploymentPlanFlattenerTests` covering empty / flat pass-through / Step Group expansion / ForEach over 3-item / empty collection warning / undefined collection warning / iteration variable substitution / index variable / display name long-value fallback / nested ForEach shadowing / lazy collection resolution / parallel ForEach StartTrigger / sequential ForEach StartTrigger / pre-M15 Guid.Empty Id legacy handling.

Suite: Data 386 + Core 223 + Server 119 (was 105 + 14 new) + Agent 128 = 856 green.
...
**Resolved 2026-05-26 in M15 follow-ups commit** — partial fix landed without needing the worker harness: `OutputVariableAccumulator` extracted from `DeploymentExecutor` (was `private static AugmentPlanWithPriorOutputs`) into a public testable helper. New `CrossIterationOutputResolutionTests` (Agent.Tests, +5) pin the contract end-to-end through the Octostache resolution chain: synthetic accumulator key (`Deploy[0]`) → merged `Octopus.Action[Deploy[0]].Output.X` variable → `#{...}` template resolves to the iteration's value. Multi-iteration distinct keys + display-name form (`Deploy [item=staging]`) + empty-outputs identity + existing-variable preservation all pinned. The full worker-pipeline test (server-side per-step boundary drain → output table write → next wave's plan augmentation) still needs the harness; that piece remains the open gap.
...
- **Rolling deployments** — M-RollingDeployments milestone. Not a new step type — just `Octopus.Action.MaxParallelism` on a `Kraken.StepGroup` driving a target-fan-out pass in the orchestrator. Requires multi-target Deployment row (the structural blocker called out in M14's "deferred" section). M15's machinery is the foundation; M-RollingDeployments adds the property-consumption pass.
  ...
  **Phase 2 — Rolling window, DONE 2026-05-26**:
  - `RollingWindowResolver` (new static helper in Server.Transport): walks each wave step's `ParentStepId` chain to the nearest `Kraken.StepGroup` with `Octopus.Action.MaxParallelism` parseable as a positive int. Returns the cap only when EVERY step in the wave shares the same rolling ancestor (mixed waves bail to "no batching" to avoid surprising operators with a partial cap). Malformed/zero/negative cap values fall back to "no cap" so a typo can't accidentally serialise the deployment to one-target-at-a-time. Pinned by 13 new tests (`RollingWindowResolverTests`).
  - `DispatchTargetWaveAcrossTargetsAsync` rewritten to batch target fan-out: when a rolling cap applies, `Chunk` the dispatchPlan into sequential batches; each batch fans out via `Task.WhenAll` (existing per-target dispatch machinery, factored into `DispatchOneBatchAsync`). Required failure inside batch K stops batches K+1..end. Non-required failures continue.
  - **Per-wave batching semantic, not group-region canary**: a cap on a `Kraken.StepGroup` limits per-WAVE target fan-out for that group's waves. Between waves, the standard sequential gate applies. NOT Octopus's "complete the group on batch N before starting batch N+1" canary semantic — that's a future iteration if operator demand surfaces. The simpler semantic is a useful concurrency cap on its own AND forward-compatible (Phase 3+ can layer canary semantics on top without changing the property).
  - **Server waves**: untouched — they ignore MaxParallelism and run once (deployment-scoped) regardless of the rolling ancestor. Phase 1b decision carries forward.
  - Two new audit event types (`Deployment.RollingBatchStarted` / `Deployment.RollingBatchCompleted`) emitted ONLY when batching actually splits (cap < dispatch count); a cap that meets/exceeds the dispatch count degrades silently to a single batch + no audit so operators don't get noise for a cap that didn't fire. Each "started" audit carries: rolling group name, batch index (1-based) / total batches, batch size, MaxParallelism, target names, wave step names. "Completed" pairs with success flag + failed target names. A summary log entry also lands on the deployment log for each batch.
  - `StepFormDialog.razor` (Kraken.StepGroup form): new "Max parallelism (rolling window)" numeric input alongside the existing ForEach panel. Bound to nullable int (blank = no cap), writes/strips `Octopus.Action.MaxParallelism` on save, hydrates on edit. Help text spells out the per-wave batching semantic + that blank/0 means no cap. InvariantCulture serialisation so the importer / resolver / UI all agree.
  - Imported Octopus rolling steps already preserve `Octopus.Action.MaxParallelism` (see `OctopusDeploymentProcessImporter` — M15.1 carry-over); Phase 2 now consumes that value end-to-end.
  - Suite: 386 + 225 + 135 + 133 = **879 green** (+13 new resolver tests on top of the 866 Phase 1b baseline).

  **Phase 3 — Polish, DONE 2026-05-26**:
  - **Per-target Required gate drop-out**: a Required step failure on target X (or an agent-offline at dispatch time) now drops X from subsequent waves instead of aborting the deployment. Other alive targets keep running. The deployment fails only when EVERY target has dropped; partial success terminates as `SucceededWithWarnings` with the drop-outs visible in the audit trail.
  - `DispatchAsync` maintains an `aliveTargets` list across waves + a `droppedTargets` accumulator. After each target wave, drop-outs from the wave's batches are applied to `aliveTargets` + emitted as `Deployment.TargetDropped` audit + log rows. Zero survivors → `FailAsync` with an aggregate "all N targets dropped" message + a legacy `Deployment.RequiredStepFailed` audit (so existing dashboards still light up).
  - `TargetWaveAggregateResult` + `BatchOutcome` refactored: replaced the singular `AbortedRequired` / `RequiredFailure` field with an `IReadOnlyList<DroppedTargetInfo>`. The Phase 2 "stop subsequent batches on first Required failure" gate is removed — drop-outs now replace it (each target's failure is local; subsequent batches still run because they target different machines). The Phase 2 rolling-window batching itself is unchanged.
  - **`Deployment.TargetDropped`** audit event: emitted per drop-out with reason (RequiredStepFailed / AgentOffline), failing step name, wave step names, and the error text. M13.B.2/3 subscribers can route on this for ops notifications.
  - **Per-target slow audit**: new `Deployment.TargetSlow` event. After deployment finalisation, each target's effective duration (max `CompletedUtc` − min `StartedUtc` across its `DeploymentStepOutcome` rows) is compared against `SlowDeploymentThresholdMinutes`; one audit per slow target. Lets operators pinpoint a single straggler that the deployment-scoped `Deployment.Slow` would have averaged-out. Same threshold setting, same "best-effort, never fails the deployment" emission policy as the existing per-deployment audit.
  - **Steps tab UI** (`DeploymentDetail.razor`): new "Target" column on the outcome grid showing per-(step, target) attribution from the Phase 1a widened key. Server-side / pre-Phase-1a outcomes (TargetId null) render "—". The header card's Target row gained multi-target pluralisation + "N targets: name1, name2, …" summary; `DeploymentService.GetAsync` now includes `Deployment.Targets.ThenInclude(Target)` so the page can map ids to names without a second round-trip.
  - **What's still deferred**: cross-target output variable attribution (`DeploymentOutputVariable` is still keyed by `(DeploymentId, StepName, Name)` — last-writer-wins across targets). Migration cost not worth it without operator demand surfacing; a follow-up M-RollingDeployments-v2 could extend the key with TargetId.
  - **Test surface (closed by E2E harness, 2026-05-26)**: when Phase 3 landed, the orchestrator's per-target drop-out had zero direct coverage and the E2E gap from M14.2 was acknowledged. A follow-up commit (this same date) shipped the harness — `tests/KrakenDeploy.Server.Data.Tests/OrchestratorHarness/` houses `OrchestratorTestHarness` + `FakeAgentHubContext` + `FakeAgent`. The harness builds the real DI container (via `AddKrakenDeployData` + a handful of transport overrides) backed by `PostgresFixture`, substitutes a fake hub so target-side dispatches resolve synchronously via configured per-target scripts, and exposes `DispatchForTestAsync` (the only internal seam — `InternalsVisibleTo` from `KrakenDeploy.Server.Transport`). Eight E2E tests pin the new behaviours: single-target happy path, multi-target fan-out, Phase 3 Required drop-out leaves survivors + SucceededWithWarnings, all-targets drop → Failed, non-required failure → SucceededWithWarnings, Phase 2 rolling window with success, Phase 2+3 rolling window with batch-1 drop-out → batch 2 still runs, per-target outcome attribution. The harness is reusable for future orchestrator work; new branches in `DispatchAsync` should ship with a test exercising them.
  - Suite: 394 + 225 + 135 + 133 = **887 green** (+8 new E2E orchestrator tests on top of the 879 Phase 2 baseline).
