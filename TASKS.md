...
| **E** — Permission-aware UI | ✅ (pending hash) | `<RequirePermission>` wraps Create/Edit/Delete buttons in Projects, Environments, Targets, TargetDetail, Tenants, Lifecycles, Runbooks, StepTemplates. `Permission` + `Shared` namespaces added to `_Imports.razor`. |
