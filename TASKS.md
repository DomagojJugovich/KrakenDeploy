...
- [x] **D-12.2: AWS S3 upload sample step package** (done) —
  ...
  - `AWSSDK.S3` v4.0.16 added to `Directory.Packages.props` under the "Sample dependencies" group (main KrakenDeploy binaries don't depend on it). The example's csproj sets `<CopyLocalLockFileAssemblies>true</CopyLocalLockFileAssemblies>` so the AWS SDK runtime DLLs land in `$(OutputPath)` and the pack target ships them in `executor/` — without this flag class libraries don't copy NuGet runtime DLLs and the agent's loader would fail to resolve `Amazon.S3.AmazonS3Client` at handler-load time. Documented as the canonical "bundle third-party SDK" pattern in `docs/step-packages.md`.
