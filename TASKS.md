...
- [ ] **7c: "Server on behalf of each deployment target" mode** — third execution location: server-side execution, but iterated once per matching target with that target's variables in scope. Needs per-target variable scoping (already partially built in `VariableService`) plus a UI affordance to declare both `RunOnServer = true` and a non-empty `TargetRoles`. Defer until 7b proves itself in real use.
- [ ] **7d: Interleaved server / target ordering** — drop the "server steps must precede target steps" restriction. Implement piecewise agent dispatch so a process like `target → server → target` runs in the user-declared order. Requires multi-round SignalR conversations with the agent and additional state-machine bookkeeping.
