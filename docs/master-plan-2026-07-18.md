...
| 4 | F3 | Settings GUI: Engine document + AgentUpdate + logging + auth + SSRF. **Landed** `524f72b` (merged `9d2a016`): the Engine settings document folds in `Engine:DefaultInterventionTimeout` (WP3, 72 h) and `Engine:MaxDeployReleaseGatedWaitDuration` (WP3-b, 7 d, paired with `MaxDeployReleaseWaitDuration`), both POSITIVE-only in the UI. `PerformanceSettings.ChangeControlAuditRetentionDays` stays on the Performance page (it already has a field there and its default of 0 = never purge is deliberately not "some number of days"), cross-linked rather than moved. | ✅ feat/settings-gui-engine | L | — |
| 4 | F4 | Remove the `ApiKey:Key` config auth path | ✅ fix/sec-remove-config-apikey | S | — |
| 4 | F5 | Machine gate reader-writer rework (mutual-consent shared mode + updater write-lock) | ✅ `feat/eng-machine-gate-rw` | M | F2 |
| 4 | F6 | Server-side per-plan target exclusion at claim time (project/runbook consent flags, FIFO by overlap, reason string, /tasks?target filter) | ✅ `feat/eng-per-plan-target-exclusion` — global claim-decision advisory lock subsumed F1's per-key lock; new `ServerTaskClaimResult.TargetBlocked` + `ServerTaskTargetExclusion`; migration `AddProjectRunbookAllowParallelTaskExecution` (WP-BASELINE dependency) | M | F1, F5 |
...
| 4 | BG1 | Topology split + on-prem blue-green (`Deployment:Topology` enum, platform release registry in KrakenDb, onprem `bluegreen` compose profile, marker-aware stop-the-world guard, maintenance-mode queue gate) | ✅ `feat/topology-onprem-bluegreen` — new neutral `KrakenDeploy.Platform` project owns `PlatformReleaseDbContext` + registry/drain classes (Saas maps the same catalog tables via an empty ownership-transfer migration; OnPremBlueGreen uses KrakenDb's `platform` schema + own history table). `database upgrade` verb + `[StopTheWorld]` attribute + version-checked Hangfire schema guard (`--stop-the-world` override); slots boot with `PrepareSchemaIfNecessary=false`. Maintenance CREATION refusal (T13) in `DeploymentService.CreateAsync` + `RunbookService.TriggerAsync` + layout banner. Drain now gates the CLAIM loop (grill B1) via `ISlotDrainGuard` at all three worker claim/resume sites, `DrainBlocked` outcome; `IsContinuationOfClaimedParent` extracted from **six** inline sites (the T12 note said three — the F6 arm and `TryResumeAsync` were missed). Blue-green smoke re-keyed to `OnPremBlueGreen` and **green + blocking** in CI; new `storage-package-watch` CI job (T10 layer 2). Merge caught two post-branch `MultiAccount:Enabled` reads F3 had added (`EffectiveSettingsService` host-settings tenancy gate, `ResolveStartupSettings`) — both re-keyed, gate now covered by tests. | L | D1; land before D4 and WP-BASELINE |
...
| 5 — product features | WP3 | Real manual intervention (pause/approve/reject). Design: `docs/design-manual-intervention.md`; engine shape in `execution-engine.md` §10. NOTE the preamble's offline-drop premise was WRONG — the offline path already REFUSED `Octopus.Manual` at bundle-gen (it never auto-approved), so that refusal was KEPT rather than weakened to log+auto-approve. `Engine:DefaultInterventionTimeout` is an F3 breadcrumb. | ✅ 3af31b9 (merged 9bf3623) | XL | D1 |
| 5 | WP3-a | **Responsible-team multiselect + process-save validation** (carved out of WP3, 2026-07-29). DONE. New `StepUiWidgets.ResponsibleTeams` multiselect over the Space's visible teams (Everyone teams excluded at source) — the FIRST server-data-backed widget in `StepUiSchemaForm`; implemented narrowly, no general options-provider mechanism, which remains a design call for the five other declared-but-unimplemented pickers. New shared `ResponsibleTeamResolver` is now the single authority for approver rules, used by BOTH `ProcessService` (refuses the save) and the orchestrator gate (fail-closed backstop, since a process can still arrive by REST or import) — so save-time and run-time rules cannot drift. Timeout is validated too: an unparseable value used to fall back silently to 72 h. | ✅ same branch | S | WP3 |
| 5 | WP3-b | **WP3 review remediation** (`/code-review` at max effort, 2026-07-30: 10 finder angles + gap sweep). 15 CONFIRMED findings against WP3/WP3-a, plus ~14 below the report cap. Highest: the offline-drop refusal compared step types case-SENSITIVELY, so a `octopus.manual` step shipped in a bundle and the offline handler passed it with NO approval record; Octostache filters (`\| ToBase64`) laundered sensitive variable values past `SecretRedactor` into the persisted, lower-privilege-readable `interruptions.instructions`; a recorded REJECTION was discarded when the gate's run-condition/role filter flipped during the pause; the freeze gate was skipped wholesale on resume; the approval test was a deny-list, so `Cancelled` (and any future status) read as APPROVED; `TimeoutHours=0` held the F1 key forever. **Decisions 2026-07-30:** mask sensitive VALUES in a cloned `VariableDictionary` before Octostache sees them (not token-text masking — indirection defeats that); keep `interruptions` CASCADE and move durability to the audit log behind a new `ChangeControlAuditRetentionDays` retention class, correcting the entity's "outlives the task" claim; REJECT `TimeoutHours=0` at save and flip `allowZero` off for `Engine:DefaultInterventionTimeout`; re-check the freeze on resume but enforce it ONLY when nothing has deployed yet (mid-work resumes finish, matching the existing "let running deployments finish" policy); keep `MaxDeployReleaseWaitDuration` at 1 h and extend the ceiling only while the child is observed `Paused`, plus a save-time warning for a gated child. Adjacent defects found: `TeamService.GetAllAsync` over-fetch FIXED here (new projected `GetSelectableApproverTeamsAsync`, so a process-editor no longer materialises every team's RBAC graph); `DeploymentPlanFlattener.ResolveConfigKey` FIXED here (it was documented case-insensitive but was ordinal, so a mis-cased `ForEach` variable name fell back silently to `item`/`index`); `PermissionEvaluator.UserIsSystemAdminAsync` ignoring `a.SpaceId` is NOT fixed and moved to **WP3-c** — WP3-b closed only the call site by passing a scope. Also verified: `IgnoreQueryFilters()` is query-compilation-WIDE in EF Core 10 (`ToQueryString`, Npgsql/PG 16), so the cross-Space sweeps are correct — documented in `execution-engine.md` §6. | ✅ 3af31b9 (merged 9bf3623) | L | WP3, WP3-a |
| 5 | WP3-c | **Polishing: the four WP3 residuals that are NOT gate bugs** (carved out of WP3-b §11, 2026-07-30). Grouped because each is small, none blocks the gate shipping, and three of the four are PRE-EXISTING defects WP3 merely illuminated — so none should be smuggled into a WP3 fix commit. **(a) `ProcessService.ValidateAsync` has NO caller anywhere** — no REST endpoint, no UI. A whole validation layer is dead code: structural validation ERRORS never reach an operator, and neither does WP3-b's gated-child advisory (which currently arrives only via `StepFormDialog`'s post-save notification calling `DescribeGatedChildAsync`). Decide whether to wire it (REST + a panel on the process page, rendering `Result.Errors` and the new non-blocking `Result.Warnings`) or to delete it and keep the per-save advisory — but do not leave it dead. **(b) A runbook run's LIVE variables can invalidate its resume checkpoint** — the only genuine bug in this group. Runbook runs resolve variables live and have no snapshot, so editing a variable that drives a `ForEach` collection changes the flattened wave count during the approval window; `RestoreFromCheckpoint` then fails the run with "the approved process no longer matches", blaming the process for a variable edit. Worse, that abort runs BEFORE the resume branch, so no `Failure`/`Always` cleanup executes — the same fail-without-cleanup shape WP3-b's freeze split was written to remove. Options: snapshot runbook variables at trigger time (a change to the runbook variable contract, so needs a decision), or detect the wave-count change and fail cleanly THROUGH the resume path with an accurate message. **(c) `PermissionEvaluator.UserIsSystemAdminAsync` ignores the role assignment's own `SpaceId`**, so `AdministerSystem` granted for ONE Space is global. Pre-existing, but WP3 is the first consumer that uses god mode to override a non-permission, per-step authorization dimension (the approver break-glass), so the blast radius grew. WP3-b closed the call site by passing a scope; the underlying method is still unscoped. Fix in `UserIsSystemAdminAsync` (honour `a.SpaceId`) or refuse to persist `AdministerSystem` on a Space-pinned assignment — and check what else short-circuits on it before choosing. **(d) Two long-standing engine notes** (`execution-engine.md` §9): server-side waves have no generic deadline ceiling, and the checkpoint is written only at a pause boundary (ordinary crash-mid-orchestration is unchanged — no lease, reconciler fails it). Both are documented-and-accepted; revisit only if a deadline ceiling for server waves becomes real work. Source: `docs/design-manual-intervention.md` §11 (now v1.5). **Decisions 2026-08-26 (implemented):** (a) WIRED, not deleted — new `GET /api/projects/{projectId}/process/validation` (`ProcessView`; Space-filtered project lookup → 404 on a foreign id) + a validation panel on the process page (Errors + Warnings, refreshed per step mutation); the per-save advisory stays, `ProcessValidator.Result` unchanged. (b) FAIL CLEANLY, no snapshot — the runbook live-variable contract is untouched; wave-count/wave-kind mismatches in `RestoreFromCheckpoint` fail THROUGH the wave loop (new `resumeInvalidated` resolver input → Failed in every mode, `Failure`/`Always` cleanup runs against the restored target state), the runbook message names a variable edit as the likely cause, the wave-KIND protection keeps failing (cleanly, own message), target-set corruption still hard-fails without cleanup. (c) `UserIsSystemAdminAsync` honours `RoleAssignment.SpaceId` — Space-pinned `AdministerSystem` is god mode only in its Space; system-wide checks (Hangfire, maintenance bypass, reach-every-Space via a null scope in `GetAccessibleSpaceIdsAsync`) need a system-scope assignment; `_systemAdminCache` re-keyed (user, Space); privilege REDUCTION only — the seeder creates only `SpaceId = null` `AdministerSystem` rows, no migration. Caller audit: all three short-circuits in `PermissionEvaluator` resolved (scope.SpaceId / scope.SpaceId / null); a pinned admin reaches its own Space through the ordinary membership sweep. (d) engine notes re-verified accurate (`execution-engine.md` v1.22). NOTE: the F3 breadcrumbs in the F3 row are untouched by this WP. | ✅ 0a0205e·db1ac18 | M | WP3-b |
...
| 5 | SC0 | **Step-systems P0 fixes (on `main`)**: empty-form Add fallback for parameterless templates; reference + collect all 17 step packages into the seed dir (glob → explicit list); `Trim()` stepTypes + CSV backfill (`Kraken.DeployPackage` unpinnable). Defects verified 2026-07-30; design source `docs/step-consolidation-plan.md` §4 SC0. Also landed: `octopus.tentaclepackage` bumped 1.0.1 (the seeder skips same-version corrected claims — idempotence trap observed live), pack target sweeps superseded archives, seed dir mirrors instead of accreting. | ✅ 6cb372c (merged 8e72c55) | S | — |
| 5 | SC | **Step-systems consolidation — packages own step types** (branch `step_consolidation`). Three-level model: `step_packages` → new `step_types` registry (SSOT for picker/schema/metadata, `ExecutionLocus` retires `ServerOnlyStepTypes`) ← `step_templates` demoted to presets; per-type `ui/schemas/{id}.json` in archives + `step_package_schemas` per version, pin-aware editor; **hard cut of `BuiltInStepSchemas`** (2 798 lines) and all `Source=BuiltIn` template rows; picker redesign (source-sectioned grid, package@version, unrunnable badges, feed-health strip); feeds multi-source + flags wired + default repo corrected (`KrakenDeploy` on GitHub is squatted → `DomagojJugovich/kraken-steps`, two-lane community repo); CI signing of built-ins, `AllowUnsignedLoads` Development-only. 16 locked decisions (grill 2026-07-30) + SC1–SC8 breakdown in `docs/step-consolidation-plan.md`. | ✅ SC1 `8b1a4af` · SC2 `966fbe9` · SC3+4a `6b2678c` · SC4b `bd5c310` · SC5 `63b91e6` · SC6 `69a156e` · SC7 `eac10b5` · SC8 docs+tests (same branch). Operator residuals: create `kraken-steps` repo + CI secrets (commands in the SC7 summary); REST integration harness for the 21 step endpoints = test debt → WP14. | XL | SC0, WP3 (editor touchpoints), D1 |
...
design decision.** *Update 2026-08-18: item 4 is now CLOSED — F5 (mutual-consent
reader-writer gate) + F6 (server-side per-plan target exclusion at claim time) both
landed; the remaining wave-gap applies to ad-hoc work only (accepted, P5). Note the
real-hub test `Second_deployment_to_the_same_machine_waits_for_the_gate` cited under
item 1 was renamed/repurposed by F6 to
`Second_exclusive_deployment_is_refused_at_claim_until_the_first_finishes` — the
gate-queue scenario it pinned is no longer reachable for two plans (the claim refuses
first); the concurrent-push property stays pinned by the `Parallel_flag…` test.* Kept
below in full because the diagnoses are the rationale for the fixes. Ordered by
original severity.
...
5. Non-additive guard (T4/T10, MARKER-AWARE): introduce a `[StopTheWorld]` migration marker
   (attribute or manifest — pick one, document it). `database --upgrade`/`setup` refuses a
   MARKED pending migration while the registry shows another non-Retired release, naming the
   migration and the runbook; purely-additive pending sets proceed — that IS the rolling
   upgrade. `--stop-the-world` override for the documented full-stop runbook. Hangfire in BG
   topologies: PrepareSchemaIfNecessary=false at slot boot (a new slot must NEVER auto-migrate
   the shared Hangfire schema while the old release is live); Hangfire schema migrates only
   inside `database --upgrade`, version-checked, treated as marked. CI: a
   Directory.Packages.props diff watch flags bumps of Hangfire.PostgreSql / storage-owning
   packages. on-prem-guide gains TWO upgrade runbooks: rolling additive (register slot →
   health → flip → drain → retire) and stop-the-world non-additive (maintenance on → drain
   all → stop → migrate → start → maintenance off).
...
#### BG2 — Windows-service slot delivery *(preamble + addendum; after BG1)*

```text
TASK: Deliver the OnPremBlueGreen topology on bare-metal Windows — per-slot Windows services
instead of compose services. Locked T6 (§5, 2026-07-28). Same registry, router, runbooks and
guards as BG1; this WP is packaging, service lifecycle and operator scripts only.

Scope:
1. Layout: N side-by-side versioned slot installs (e.g. C:\KrakenDeploy\slots\1..3) each
   registered as a Windows service (KrakenDeploySlot1..N) with per-slot appsettings (slot
   port, topology, shared connection string); Router as a service (KrakenDeployRouter) on the
   operator-facing port; ONE shared Server:DataPath directory + DP cert for all slots
   (co-located slot rule); shared Postgres.
2. PowerShell operator scripts — ALL state-changing scripts support -WhatIf (house rule):
   New-KrakenSlotInstall (provision a slot service), Install-KrakenRelease -Slot N -Package
   (unpack new version into the slot dir, stamp release id, invoke `releases register`),
   Invoke-KrakenFlip / -Drain / -Retire (wrap the CLI verbs), Invoke-KrakenStopTheWorld
   (maintenance on → drain all → stop services → database --upgrade --stop-the-world → start
   → maintenance off). Scripts are Windows PowerShell 5.1-compatible (state the shell).
3. Windows specifics: service account guidance (gMSA preferred, LocalSystem fallback — note
   %TEMP% ACL implications from the F7 secrets finding); DataPath ACL locked to the service
...
- DECIDED (2026-07-06, do not re-litigate): approvers = step-defined responsible team(s),
  ...
  [WP3 OUTCOME 2026-07-29: delivered, including the multiselect (see WP3-a in §4 — the
  field first shipped as free text and was completed in the same branch).]
...
| Version | Date | Change |
|---|---|---|
| 2.8 | 2026-08-26 | **WP3-c landed** (`feat/wp3-c-polish`, 0a0205e + review remediation db1ac18). The four WP3 residuals closed: (a) process validation wired (REST + process-page panel), (b) invalidated resume checkpoints fail THROUGH cleanup with a kind-stable wave guard and a persisted `ResumeInvalidated` checkpoint flag, (c) `UserIsSystemAdminAsync` honours `RoleAssignment.SpaceId` (answered from the shared assignment cache) + `TeamService` refuses Space-pinned system-only roles, (d) engine notes re-verified. Max-effort `/code-review` remediation: approved gate decisions now outrank the run-condition filter and are identity-checked by name; the gated-child advisory no longer reads past the Space filter; docs design-manual-intervention 1.5/1.6, execution-engine 1.22/1.23. |
| 2.7 | 2026-08-18 | **F6 landed** (`feat/eng-per-plan-target-exclusion`). Server-side per-plan target exclusion at claim time, per the locked P1–P7 model: new `projects.allow_parallel_task_execution` + `runbooks.allow_parallel_task_execution` (migration `AddProjectRunbookAllowParallelTaskExecution` — WP-BASELINE dependency), OR-composed with the target flag into `DeploymentPlan.AllowParallelTaskExecution` at plan build and evaluated per target at claim. `ServerTaskLease.TryClaimAsync` now wraps BOTH kinds in one transaction under ONE global constant advisory lock (`ClaimDecisionLockKey` — replaces and subsumes F1's per-key lock; the F1 predicate is unchanged); new `ServerTaskClaimResult.TargetBlocked`; shared machinery in `ServerTaskTargetExclusion` (conflict query, ancestor-chain exemption, blocker description, advisory-locked one-time first-deferral log line). Reason surface on both detail pages ("Waiting for target X — busy with #N (title); M tasks ahead."), Tasks page `?target=` assignment-joined filter + TargetDetail deep link, project/runbook consent checkboxes with the target-flag warning pattern. §F2-followups item 4 CLOSED (wave-gap residual now ad-hoc-only, P5). Tests: new `TargetExclusionTests` (10, real Postgres, incl. the global-lock race and the exactly-one-log-line race); two pre-F6 tests re-pointed (`Pausing_frees_the_node_task_gate_slot…` second box; `Second_deployment…waits_for_the_gate` → `…is_refused_at_claim…`). CONTRACT CHANGE: none on the agent wire; EF migration (two columns). Docs: execution-engine 1.20, node-concurrency 1.6, disconnect-reconciliation 1.3. |
| 2.6 | 2026-07-30 | **Step-systems consolidation planned + landed** (grill session, 16 locked decisions → `docs/step-consolidation-plan.md`). Context: a23c691 ("45 built-in step types") landed with an empty-form Add bug, a 10-package seed gap, and an untrimmed-stepTypes claim bug; verification also established `BuiltInStepSchemas`/template-seed/manifest triple-list drift, dead `UiSchemaJson` plumbing (no package ships a schema), squatted `KrakenDeploy` GitHub name behind the catalog 404, unwired `feeds.*` flags, and unsigned built-ins. New rows **SC0** (P0 fixes on main, S) and **SC** (consolidation, XL: `step_types` registry as SSOT, presets model, pin-aware schemas, hard cut of hardcoded schemas, picker redesign, multi-feed catalogs, two-lane community repo `DomagojJugovich/kraken-steps`, CI signing). SC added to WP-BASELINE dependencies. SC0–SC8 implemented on `step_consolidation`, then two max-effort `/code-review` rounds (31 findings fixed: old-agent manifest skew, runbook-run pin vs the SD-11 sweep, offline-drop bypassing the SC4-b type guard, pin-blind guard/wave routing, an upload-time reserved-type guard closing the built-in takeover on the PIN path, and the space-scoped release scan in `UninstallAsync`). Merged to `main` with the branch's `AgentContract` 2→3 bump DROPPED — main had already taken 3 (F5) and 4 (handshake header); the SC1 manifest-shape break needs no separate bump because `AgentContract` and `StepPackageManifest` ship in one assembly, so v4 already refuses any pre-SC1 agent. |
| 2.5 | 2026-07-29 | **F5 review follow-up** (max-effort `/code-review`, 11 finder angles). 15 confirmed findings fixed; the gate state machine itself survived four independent adversarial passes. Substantive changes beyond the original WP: shared co-running BOUNDED by the new `Agent:MaxConcurrentSharedWork` (the rework had dropped the old one-at-a-time cap with nothing in its place); the self-upgrade now asks the server `GET /api/agents/task-in-flight` fail-closed before extracting, because the gate's per-wave unit let a swap `Environment.Exit` between two waves of a live plan — a gap a SERVER wave can hold for hours, and one writer fairness makes the updater win deterministically; permanent refusals moved before the gate (a `dotnet KrakenDeploy.Agent.dll` launch, which is what `Dockerfile.agent` uses, would otherwise freeze the box every tick forever for an update it always refuses); maintenance window re-checked after queueing; ROLLBACK swap also gated; `Agent:Update:*` validated at startup with `SwapGateTimeout` defaulted to 2 min so it stays under both `CheckInterval` and `Adhoc:MaxTotalDuration`; dispatch eligibility gated on a PASSED `RegisterAsync`, closing the connect→register window in which a v2 agent could be sent work and read `true` as a full bypass. **ACCEPTED RISK recorded** (P5 kept literal, Domagoj 2026-07-29): the AI flow sends `true` for `AdhocMode.Mutating` too, so two mutating sessions co-run on one box — documented in `docs/adhoc-actions.md` §2 with a WP16 rider. Note the review REFUTED one of its own findings: a token from a disposed `CancellationTokenSource` does not throw on .NET 10, so the stranded-waiter mechanism it hypothesised does not exist; the guard was kept as defence-in-depth with an accurate comment. |
| 2.4 | 2026-07-29 | **F5 landed** (`feat/eng-machine-gate-rw`). `MachineExecutionGate` is a fair async reader-writer lock; `AllowParallelTaskExecution` downgrades to SHARED instead of bypassing (P2 mutual consent); the self-upgrade swap window takes the WRITE side (P8), bounded by the new `Agent:Update:SwapGateTimeout`. Ad-hoc gate mode became per-RUN (P5 read-always for the AI flow) — `AdhocDispatcher.DispatchAsync` now takes a `bool` rather than a per-target map, and `AdhocSessionService` stopped querying `deployment_targets` for it. Item 3's "acquire READ always" is superseded by item 5 (v1.3, 2026-07-27): the agent maps the retained field both ways so WP16's console default can be EXCLUSIVE. CONTRACT CHANGE: `AgentContract.CurrentVersion` 2 → 3, no shape change — the v2 reading of `true` is a full bypass, which on the read-always AI path would leave every approved script ungated, so the skew is refused at registration. F6 is now unblocked. |
| 2.3 | 2026-07-29 | Header-only bump that never got a history row (recorded retroactively by the F5 review's docs pass). |
