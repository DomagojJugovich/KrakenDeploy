...
| 4 | F3 | Settings GUI: Engine document + AgentUpdate + logging + auth + SSRF. **Breadcrumbs to fold in when this lands** (each shipped as a config-file knob on `EngineOptions` because F3 was not ready, mirroring what F2 did with `MaxTargetQueueWait` — house rule 11 wants them in the Engine settings document): `Engine:DefaultInterventionTimeout` (WP3, 72 h — NOTE the validator refuses zero, because an unexpiring manual-intervention gate is skipped by the timeout sweeper while its task keeps holding the F1 `(project, environment, tenant)` key, so one unanswered gate blocks every later release of that pair); `Engine:MaxDeployReleaseGatedWaitDuration` (WP3-b, 7 d — the hard backstop for a `DeployRelease` parent whose child pauses at a gate, paired with `MaxDeployReleaseWaitDuration` which bounds only NON-paused waiting). Both must stay POSITIVE in whatever UI F3 builds; a zero in either reintroduces an unbounded hold. Also surface `PerformanceSettings.ChangeControlAuditRetentionDays` alongside them if the Engine document ends up owning retention windows — it already has a Performance-page field, but it is the one retention knob whose default (0 = never purge) is deliberately NOT "some number of days". | ⬜ | L | — |
