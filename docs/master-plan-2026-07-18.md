...
| 5 | WP3-b | **WP3 review remediation** (`/code-review` at max effort, 2026-07-30: 10 finder angles + gap sweep). 15 CONFIRMED findings against WP3/WP3-a, plus ~14 below the report cap. Highest: the offline-drop refusal compared step types case-SENSITIVELY, so a `octopus.manual` step shipped in a bundle and the offline handler passed it with NO approval record; Octostache filters (`\| ToBase64`) laundered sensitive variable values past `SecretRedactor` into the persisted, lower-privilege-readable `interruptions.instructions`; a recorded REJECTION was discarded when the gate's run-condition/role filter flipped during the pause; the freeze gate was skipped wholesale on resume; the approval test was a deny-list, so `Cancelled` (and any future status) read as APPROVED; `TimeoutHours=0` held the F1 key forever. **Decisions 2026-07-30:** mask sensitive VALUES in a cloned `VariableDictionary` before Octostache sees them (not token-text masking — indirection defeats that); keep `interruptions` CASCADE and move durability to the audit log behind a new `ChangeControlAuditDays` retention class, correcting the entity's "outlives the task" claim; REJECT `TimeoutHours=0` at save and flip `allowZero` off for `Engine:DefaultInterventionTimeout`; re-check the freeze on resume but enforce it ONLY when nothing has deployed yet (mid-work resumes finish, matching the existing "let running deployments finish" policy); keep `MaxDeployReleaseWaitDuration` at 1 h and extend the ceiling only while the child is observed `Paused`, plus a save-time warning for a gated child. Adjacent, NOT fixed here: `TeamService.GetAllAsync` has no permission check, `PermissionEvaluator.UserIsSystemAdminAsync` ignores `a.SpaceId`, `DeploymentPlanFlattener.ResolveConfigKey` is documented case-insensitive but is ordinal. Also verified: `IgnoreQueryFilters()` is query-compilation-WIDE in EF Core 10 (`ToQueryString`, Npgsql/PG 16), so the cross-Space sweeps are correct — documented in `execution-engine.md` §6. | 🔄 same branch | L | WP3, WP3-a |
