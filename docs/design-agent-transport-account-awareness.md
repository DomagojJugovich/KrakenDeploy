...
| **Status** | `Review` — server-side implemented (phases 0–4); build/unit/smoke-verified, live-agent verification pending |
...
Implementation status (2026-06-26): phases **0–5 done and build/unit/smoke-verified**; phases 6–8 pending (Phase 6 covered by the filter backstop; Phase 5 `AdhocDispatcher` guard deferred). Per-phase detail is tracked in [`saas-phase3-account-awareness.md`](saas-phase3-account-awareness.md) P3-8. Notably the host-derived design made phases 1 and 4 zero-change (the wizard already emits the operator's subdomain via `Nav.BaseUri`; gRPC inherits the account from the middleware), and the agent-side concern reduced to one new filter + one hub edit.
...
5. **Harden dispatch:** assert connected target's resolved account == dispatch `_dispatchAccountId`; mismatch → offline drop-out. Same guard in `AdhocDispatcher`. **DONE** for `DeploymentWorker` + `RunbookRunWorker` (registry records the connection's account; workers assert it at dispatch). `AdhocDispatcher` deferred (singleton on the interactive path — needs the account threaded into `DispatchAsync`).
6. **Tighten `AccountResolutionMiddleware`** so agent paths (`/hubs/agent`, `/kraken.*`) require a resolved account in multi-account mode (no apex pass-through).
...
7. **Edge:** wildcard site block (`*.{$DOMAIN}`) + DNS-01 wildcard cert in the Caddy config; confirm `Host` reaches the backend on `/hubs/*` and `/kraken.*` with a connected agent.
8. **Isolation tests (P3-7 overlap):** agent on `acme.<base>` with a `globex` `targetId` → rejected; agent on the bare host in multi-account → rejected; deployment dispatched to the wrong account → drop-out, no cross-account push.
